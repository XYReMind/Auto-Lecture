...
double keep_lane_cost(bool car_head, int my_lane, double closest_front_dist, int cost_collision) {
  ...
  if (car_head) {
    ...
    if (closest_front_dist < 150) {// If there are cars within 150 m, the chance of a collision is high
        cost += 100;
    } else {
        cost += 0;
    }
...
double turn_right_cost(bool car_right, int my_lane, 
                      double closest_rightfront_dist, 
                      double closest_rightback_dist,
...
double turn_left_cost(bool car_left, int my_lane, 
                      double closest_leftfront_dist, 
                      double closest_leftback_dist,
        ...
        if (s != "") {
            ...
            if (event == "telemetry") {
                ...
                for (int i = 0; i < sensor_fusion.size(); ++i) {
                    ...
                    if (others_lane == my_lane){
                      if( (fusion_s > car_s) && (fusion_s - car_s < 40)){
                        car_head=true;
                        if (fusion_speed) {
                           //change the unit 
                          closest_front_vec = ms_to_mph(fusion_speed); 
                        }
                      }
                      if (fusion_s-car_s > 0) {
                        closest_front_dist = min(fusion_s-car_s, closest_front_dist); 
                        //find the minimal distance to the closest front car
                      }
                      if( (fusion_s > car_s )&& (fusion_s - car_s < 15)){ 
                      ...
                      if((car_s + 30 > fusion_s)&&(car_s - 30 < fusion_s)){ 
                        // safe traffic-gap (-30,30)
                        car_left= true;
                      }
                      if (fusion_s-car_s > 0) {
                          if (fusion_s-car_s<closest_leftfront_dist){
                            closest_leftfront_dist=fusion_s-car_s;
                            if(fusion_s-car_s<10){
                              // calculate the lateral distance between the two cars
                              closest_left_d=car_d-fusion_d;
                            }
                          }
                      } else {
                        // find the minimal distance to the closest rightback car
                        closest_leftback_dist = min(abs(fusion_s-car_s),closest_leftback_dist);
                      }
                    }else if (others_lane == my_lane + 1) {
                      // in right lane
                      if( (car_s + 30 > fusion_s) && (car_s - 30 < fusion_s) ){
                      ...
                      if (fusion_s-car_s > 0) {
                          if (fusion_s-car_s<closest_rightfront_dist){
                            closest_rightfront_dist=fusion_s-car_s;
                            if(fusion_s-car_s<10){
                              closest_right_d=fusion_d-car_d;// calculate d between the two cars
                            }
                          }
                    
                      } else {
                        closest_rightback_dist = min(abs(fusion_s-car_s),
